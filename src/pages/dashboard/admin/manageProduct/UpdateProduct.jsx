...
const UpdateProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // ✅ Color is only required if "Georgette  Hijabs" is selected
        const isColorRequired = categories.includes('Georgette  Hijabs');
        if (isColorRequired && !color) {
            setNotification({ show: true, type: 'error', message: 'Color is required for Georgette  Hijabs category' });
            return;
        }
                ...
                <div>
                    ...
                    {isColorRequired && (
                        <p className="text-xs text-amber-600 mt-1">
                            ⚠️ Color is required when "Georgette  Hijabs" is selected
                        </p>
                    )}
